...
export default function LoginPage() {
  const { login, signup, forgotPassword, resetPassword, backendOnline } =
    useAccounting();
  ...
  const handleSignUp = async (e) => {
    ...
    try {
      ...
      setErrorMessage(
        err.message || "Signup failed. Please check your credentials.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleVerifyAccount = async (e) => {
    ...
    try {
      ...
      setErrorMessage(
        err.message || "No user account found matching this identifier.",
      );
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="w-full max-w-4xl relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 my-auto">
        ...
        <div className="lg:col-span-5 flex flex-col justify-between p-8 rounded-3xl bg-white border border-[#E3E7EA] shadow-xs relative overflow-hidden">
          <div className="space-y-6 relative z-10">
            ...
            <div className="flex items-center space-x-3.5">
              ...
              <div>
                <h1 className="text-xl font-bold tracking-tight text-[#0B2A4A] font-display">
                  Urban Furniture
                </h1>
                <p className="text-xs text-[#667482] font-medium tracking-wide">
            ...
            <div className="space-y-3 pt-2">
              ...
              <p className="text-xs text-[#667482] leading-relaxed">
                Seamless real-time transactions, automated sales and purchase
                workflows, dynamic budget tracking, and balanced financial
                ledger reports.
              </p>
            </div>
            ...
            <div className="space-y-3 pt-2">
              <div className="flex items-center space-x-3 text-xs text-[#17212B] bg-[#FAFAF8] p-3 rounded-xl border border-[#E3E7EA]">
                ...
                <span className="font-medium">
                  Strict Double-Entry Balanced Ledger
                </span>
              </div>
              <div className="flex items-center space-x-3 text-xs text-[#17212B] bg-[#FAFAF8] p-3 rounded-xl border border-[#E3E7EA]">
                <CheckCircle2 className="w-4 h-4 text-[#0B2A4A] shrink-0" />
                <span className="font-medium">
                  Real-Time Inventory & Stock Movement
                </span>
              </div>
              <div className="flex items-center space-x-3 text-xs text-[#17212B] bg-[#FAFAF8] p-3 rounded-xl border border-[#E3E7EA]">
                <CheckCircle2 className="w-4 h-4 text-[#C98232] shrink-0" />
                <span className="font-medium">
                  Dynamic Budgets & Analytic Accounts
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-7 bg-white border border-[#E3E7EA] rounded-3xl p-6 sm:p-10 shadow-xs flex flex-col justify-between space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-[#667482] mt-0.5">
                ...
                {authMode === "forgot" &&
                  "Enter your Login ID or registered Email ID"}
                {authMode === "reset" &&
            ...
            {/* Live Backend Badge */}
            <div
              className={`flex items-center space-x-1.5 px-3 py-1 rounded-full text-[11px] font-mono font-semibold border ${
                backendOnline
                  ? "bg-[#EAF7F0] border-[#A3E6C2] text-[#18794E]"
                  : "bg-[#FFF6DF] border-[#FBD38D] text-[#B7791F]"
              }`}
            >
              <span
                className={`w-2 h-2 rounded-full ${backendOnline ? "bg-[#18794E] animate-pulse" : "bg-[#B7791F]"}`}
              ></span>
              <span>{backendOnline ? "MySQL Connected" : "Connecting to MySQL..."}</span>
            </div>
          </div>
          ...
          {authMode === "signup" && (
            <form onSubmit={handleSignUp} className="space-y-3.5">
              <div className="space-y-1">
                <label className="text-xs font-semibold text-[#17212B] block">
                  Login ID{" "}
                  <span className="text-[#8A96A3] font-normal">
                    (6–12 characters)
                  </span>
                </label>
                <div className="relative">
              ...
              <div className="p-3 bg-[#FAFAF8] rounded-xl border border-[#E3E7EA] space-y-1 text-[11px] text-[#667482]">
                ...
                <div className="grid grid-cols-2 gap-1.5 pt-0.5">
                  <span
                    className={`flex items-center space-x-1.5 ${currentPwdRules.length ? "text-[#18794E] font-semibold" : "text-[#8A96A3]"}`}
                  >
                    <CheckCircle2 className="w-3.5 h-3.5" />{" "}
                    <span>&gt; 8 characters</span>
                  </span>
                  <span
                    className={`flex items-center space-x-1.5 ${currentPwdRules.lower ? "text-[#18794E] font-semibold" : "text-[#8A96A3]"}`}
                  >
                    <CheckCircle2 className="w-3.5 h-3.5" />{" "}
                    <span>Lowercase (a-z)</span>
                  </span>
                  <span
                    className={`flex items-center space-x-1.5 ${currentPwdRules.upper ? "text-[#18794E] font-semibold" : "text-[#8A96A3]"}`}
                  >
                    <CheckCircle2 className="w-3.5 h-3.5" />{" "}
                    <span>Uppercase (A-Z)</span>
                  </span>
                  <span
                    className={`flex items-center space-x-1.5 ${currentPwdRules.special ? "text-[#18794E] font-semibold" : "text-[#8A96A3]"}`}
                  >
                    <CheckCircle2 className="w-3.5 h-3.5" />{" "}
          ...
          {authMode === "reset" && (
            <form onSubmit={handleResetPassword} className="space-y-4">
              <div className="p-3 bg-[#EEF4F8] border border-[#D8E1E8] rounded-xl text-xs text-[#0B2A4A]">
                ...
                <strong>{verifiedUser?.name || forgotIdentifier}</strong>
              </div>
          ...
          <div className="pt-4 border-t border-[#E3E7EA] flex items-center justify-center space-x-2 text-xs text-[#8A96A3]">
            ...
            <span>
              Secure session authenticated via JWT and MySQL role-based access
              control.
            </span>
          </div>
        </div>
